...
export async function POST(
  request: NextRequest,
  context: { params: Promise<{ orderId: string; notificationId: string }> },
) {
  try {
  ...
  const { orderId, notificationId } = await context.params;

  try {
    const result = await retryOrderNotification(orderRepository, {
      orderId,
      notificationId,
    });
    return NextResponse.json(result);
  } catch (error) {
    if (error instanceof OrderNotificationRetryError) {
